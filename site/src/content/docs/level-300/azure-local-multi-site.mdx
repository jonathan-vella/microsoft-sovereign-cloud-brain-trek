...
:::tip[📝 Key Distinction]
Azure Local does NOT support stretch clusters (single cluster spanning multiple sites). This page covers **multi-cluster** architectures where each site has its own Azure Local cluster, connected through replication and Arc management.

For **rack-level** high availability within a single cluster, see [Rack-Aware Clustering](/level-200/azure-local-ha-patterns/#rack-aware-clustering-preview).
:::


<DiagramContainer title="Multi-Site Replication Topology">

![Azure Local Multi-Site Architecture showing primary, DR, and edge sites with replication topology](/microsoft-sovereign-cloud-brain-trek/images/level-300/azure-local-multisite.svg)
...
**See also:** [Connected Mode Architecture](/level-300/azure-local-advanced-connected/) | [Networking Deep Dive](/level-300/azure-local-networking-advanced/)
