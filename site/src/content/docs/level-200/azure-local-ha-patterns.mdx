...
<DiagramContainer title="HA Topology Options" defaultOpen>
...
:::tip[📝 November 2025 Update]
Stretch clusters (cross-site synchronous replication within a single cluster) are **not supported** in Azure Local. For multi-rack high availability, use **Rack-Aware Clustering** (Preview). For multi-site DR, use Azure Site Recovery or Storage Replica between separate clusters.
:::
...
:::note[📝 Preview Limitation]
Rack-aware clustering does not support External Storage Area Networks (SAN). All storage must be local to cluster nodes.
:::
